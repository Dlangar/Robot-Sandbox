...
[ExecuteInEditMode]
public class MechCamera : PivotBasedCameraRig
{
   [SerializeField]
   private Transform m_RotateObj;                  // We use the targetObj for position, but we take our rotation from the rotateObj
   ...
   [SerializeField] private bool m_FollowTilt = true; // Whether the rig will tilt (around X axis) with the target.77
   ...
   protected override void FollowTarget(float deltaTime)
   {
      ...
      if (m_RotateObj != null)
         targetForward = m_RotateObj.forward;
/*
      if (m_FollowVelocity && Application.isPlaying)
      {
         // in follow velocity mode, the camera's rotation is aligned towards the object's velocity direction
         // but only if the object is traveling faster than a given threshold.
         if (targetRigidbody != null && (targetRigidbody.velocity.magnitude > m_TargetVelocityLowerLimit))
         {
      ...
      */
      ...
      var desiredRotation = Quaternion.LookRotation(targetForward, Vector3.up);
      transform.rotation = Quaternion.Lerp(transform.rotation, desiredRotation, m_TurnSpeed*m_CurrentTurnAmount*deltaTime);
   ...
   public void SetRotateObj(Transform rotateObj)
   {
      m_RotateObj = rotateObj;
   }
